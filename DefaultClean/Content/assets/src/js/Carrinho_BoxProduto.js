...
function formTemAlertaAntesDeValidar(alerta) {
    ...
    if (alertAtualAntesDeValidar) {
        return {
            "cepInvalido": function() {$.alertpadrao({ type: 'html', text: "Esse CEP é inválido. Por favor, insira um CEP válido. Para mais informações, entre em contato com o nosso SAC: (11) 3990-5089", addClass: "dg-negativo" })},
            "cepEndNaoEncontrado": function() {$.alertpadrao({ type: 'html', text: "Dados de endereço não encontrado. Por favor, procure o nosso SAC para mais informações: 11 3990-5089", addClass: "dg-negativo" })},
    ...
    var BoxProduto = {
        ...
        eventos: function() {
            ...
            $("body").on('click', '.BtComprarProduto', function () {
                ...
                if (paginaAtual === 'Checkout') { loadingContentBoth(true); }
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if ($(".titlecountcart").length > 0) {
                ...
                $(".titlecountcart").attr("title", $(".titlecountcart").attr("dataformat").format(response.ItensCart.length)).find(".dg-header-carrinho-numero").text(response.ItensCart.length);
            }
...
function WaiterController() {
    if (waiter === 0) {
        ...
        waiter = 0;
        maxWaiter = 0;
        $.alertpadrao({ text: "Produtos adicionados ao carrinho!", mode: "alert", addClass: "dg-positivo" });
        GetCarrinho(null);
    } else {
        waiter++;
    }
    ...
    return $.ajax({   // o return obrigatorio do ajax 
        ...
        success: function (response) {
            ...
            if (Action == "carregaCarrinhoIni") {
                ...
                if (response.ItemFeedbacks.length > 0) {
                    response.ItemFeedbacks.forEach(function (elemento) {
                        ...
                        if (elemento.ID_Status == 6) {
                            $.alertpadrao({ type: 'html', text: "Sem Estoque: quantidade em estoque: " + elemento.Estoque, addClass: "dg-negativo" });
            ...
            if (response.ItemFeedbacks.length > 0) {
                ...
                $.alertpadrao({ type: 'html', text: 'ops deu algo errado!', addClass: "dg-negativo" });
            }

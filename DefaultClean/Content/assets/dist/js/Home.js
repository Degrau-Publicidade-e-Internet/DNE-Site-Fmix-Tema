...
$(document).ready(function() {
    ...
    if ($('#chaveExpirada').length > 0) {
        $.alertpadrao({
            type: 'html',
            text: "Infelizmente o link que você tentou acessar já não é mais válido. Por favor, faça login e efetue a validação do cadastro por e-mail ou SMS.",
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.Msg == "OK") {
                        ...
                        $.alertpadrao({
                            type: 'html',
                            text: "Erro ao tentar localizar seu cadastro, por favor tente novamente. ",
                            addClass: "dg-negativo"
                        });
                    }
                },
                    ...
                    if (response !== null && response !== undefined) {
                        if (response.ID_Status > 0) {
                            if (response.ID_Status == 2) {
                                $.alertpadrao({
                                    type: 'html',
                                    text: "Essa recompra já foi processada.",
                                    addClass: "dg-negativo"
                                });
                            } else {
                                if (response.Erros.length > 0) {
                                    ...
                                    $.alertpadrao({
                                        type: 'html',
                                        text: "Erro ao tentar recuperar seu pedido.",
                                        addClass: "dg-negativo"
                                    });
                                }
                            }
                            ...
                            if (response.TSource.ID_Pedido > 0) {
                                ...
                                var ResultadoCont = TrimPath.processDOMTemplate("#templateModal1Clique", {
                                    Retorno: response.TSource
                                })
    ...
    if ($("#KeyCarrinhoAbandono").length > 0) {
        ...
        $.ajax({
            type: 'POST',
            data: JSON.stringify(item),
            url: "/api/verificaClienteLoginCarrinhoAmbandono/" + $("#KeyCarrinhoAbandono").val(),
            contentType: 'application/json; charset=utf-8',
            headers: addAntiForgeryToken({}), //<< obrigatorio pessoal
            dataType: 'json',
            success: function(response) {
            ...
            success: function(response) {
                ...
                $(".for-carrinho-recompra .mais-qtd").click(function() {
                    ...
                    if (Qtd > estoque) {
                        $.alertpadrao({
                            type: 'html',
                            text: "Estoque: " + verEstoque.Estoque,
                            addClass: "dg-negativo"
                        });
                        Qtd = estoque;
                    }

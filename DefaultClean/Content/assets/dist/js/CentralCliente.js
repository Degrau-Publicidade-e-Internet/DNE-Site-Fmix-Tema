...
var ClienteCentral = {
    ...
    MostraAba: function(aba, response, iDPedido, pg = 1) {
        return {
            meusPedidos: function() {
                ...
                var Resultado = TrimPath.processDOMTemplate("#templatePedidosConteudo", {
                    RetornoPedidos: response.Lista
                });
                ...
                if (paginacao.length > 0 && paginacao != null) {
                    paginacao.forEach(function(x) {
                        var ativo = "";
                        if (x.IsSelecionado == true) ativo = 'dg-ativo';
                        var _li = '<li>';
            ...
            gerenciarAssinaturas: function() {
                $(".dg-usuario-template").html("");
                $('.dg-usuario-template .pageFooter').html('');
                var Resultado = TrimPath.processDOMTemplate("#templateAssinaturasConteudo", {
                    Assinaturas: response.Lista
                });
                $(".dg-usuario-template").html(Resultado);
            ...
            cartoesCadastrados: function() {
                ...
                var Resultado = TrimPath.processDOMTemplate("#cartoesCadastrados", {
                    Cartoes: response.Lista
                });
                $(".dg-usuario-template").html(Resultado);
            ...
            editarEnderecos: function() {
                ...
                var Resultado = TrimPath.processDOMTemplate("#editarEnderecos", {
                    Enderecos: response.Lista
                });
                $(".dg-usuario-template").html(Resultado);
                $('.mostraItens').click(function() {
                    $(this).parent().toggleClass('dg-option-opened');
            ...
            editarConsentimentos: function() {
                if (response.Lista.News == true) {
                    $('#ofertaEmail').prop('checked', true)
                }
                if (response.Lista.SMS) {
                    $('#ofertaSms').prop('checked', true)
                }
                if (response.Lista.Redes_Sociais) {
                    $('#ofertaRedesSociais').prop('checked', true)
                }
                if (response.Lista.WhatsApp) {
                    $('#ofertaWhatsapp').prop('checked', true)
                }
            },
            produtosFavoritos: function() {
                ...
                if ($('.dg-usuario-template #HtmlUsuarioFavLista .dg-favoritos-boxproduto-lista').children().length === 0) {
                    $('.dg-usuario-template #HtmlUsuarioFavLista .dg-favoritos-boxproduto-lista').addClass('dg-center-text').text('<h1 class="dg-titulo">Gostou de algum produto? Marque o como favorito e o salve aqui nesta lista! Você será notificado caso seus favoritos fiquem em promoção ou quando voltarem ao estoque.</h1>');
                }
            ...
            verPedidos: function() {
                var Resultado = TrimPath.processDOMTemplate("#templatePedidosDinamicoConteudo", {
                    RetornoPedido: response.Lista,
                    Pagina: ClienteCentral.pgPedido
                });
                $(".dg-usuario-template").html(Resultado);
    ...
    editarEnderecoCliente: function(id) {
        getEndereco(function() {
            if ($('.jsAssinaturaPainel').length === 0) {
                ...
                abrirModal({
                    titulo: '<span icon-aba="editarEnderecos" class="dg-usuario-sidebar-icon"></span>' + titulo + ' <strong>Endereço</strong>',
                    id: 'inserirEndereco',
                    classe: 'ModalAssinaturaEndereco ModalAssinaturaEnderecoCadastro'
                });
    ...
    editarAssinatura: function(id) {
        getAssinatura(function(info) {
            ...
            if (id !== 0) {
                ...
                $('.dg-usuario-main .dg-titulo').html('<div><span icon-aba="gerenciarAssinaturas" class="dg-usuario-sidebar-icon"></span> Assinatura nº <strong>' + id + '</strong>:</div>');
            }
    ...
    excluiAssinatura: function(id) {
        $.alertpadrao({
            type: 'html',
            mode: "confirm",
            text: "Confirma a exclusão dessa assinatura?",
            addClass: "dg-atencao",
            CallBackOK: function() {
    ...
    gravaAssinatura: function(id) {
        $.alertpadrao({
            type: 'html',
            mode: "confirm",
            text: "Deseja gravar essa assinatura?",
            addClass: "dg-atencao",
            CallBackOK: function() {
    ...
    atualizaAssinatura: {
        removeProdutoLista(id) {
            $.alertpadrao({
                type: 'html',
                mode: "confirm",
                text: "Confirma a exclusão desse produto?",
                addClass: "dg-atencao",
                CallBackOK: function() {
        ...
        populate: function(obj, template, append) {
            console.log(obj);
            var Resultado = TrimPath.processDOMTemplate(template, {
                info: obj
            });
    ...
    modalCartaoAssinatura: function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                var Resultado = TrimPath.processDOMTemplate("#templateCartaoAssinaturaOpcoes", {
                    Cartoes: response.Lista,
                    BandeiraSelecionada: $('.jsCardSelected').attr('data-bandeira'),
                    CartaoSelecionado: $('.jsCardSelected').attr('data-id')
                });
    ...
    modalFrequenciaAssinatura: function() {
        ...
        var Resultado = TrimPath.processDOMTemplate("#templateFrequenciaAssinaturaOpcoes", {
            frequencia: info
        });
        $('.jsAssinaturaFrequenciaOption').html(Resultado);
        $('.jsSelectFrequenciaAssinatura').on('change', function() {
            var value = $(this).find('option:selected').val();
            ClienteCentral.atualizaAssinatura.dados.frequencia({
                dias: value
            });
            fecharModal($('.dg-produto-assinatura-modal')[0]);
            ClienteCentral.validaAntesDeMostrarGravarAssinatura();
    ...
    modalEnderecoAssinatura: function() {
        ...
        item.ID_Usuario = parseInt($('#ID_Cliente').val());
        abrirModal({
            id: "templateEnderecoAssinatura",
            classe: "ModalAssinaturaEndereco"
        });
        $.ajax({
            type: 'POST',
            data: JSON.stringify(item),
                ...
                $('.dg-usuario-editar-enderecos-escolha').on('click', function() {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            ClienteCentral.atualizaAssinatura.dados.endereco({
                                id: obj.Endereco.ID_Endereco,
                            ...
                            if (obj.IsEC87) {
                                ...
                                $.alertpadrao({
                                    type: 'html',
                                    text: "Devido a emenda constitucional 87 este pedido terá seu valor acrescido em 4%."
                                });
    ...
    mostraProdutoAssinatura: function(info) {
        var Resultado = TrimPath.processDOMTemplate("#templateResultadoProdutoAssinatura", {
            Produto: info
        });
    ...
    validaAntesDeMostrarGravarAssinatura: function() {
        ...
        if ($('.dg-cliente-assinatura__list-product li[data-produto-id]').length > 0 && $('.jsFrequenciaSelected[data-frequencia]').length > 0 && $('.jsEnderecoSelected[data-end-id]').length > 0 && $('.jsCardSelected[data-id]').length > 0) {
                            ...
                            AjaxAutoComplete = $.ajax({
                                ...
                                success: function(response) {
                                    if (response.Lista.length > 0) {
                                        var Resultado = TrimPath.processDOMTemplate("#templateResultadoBuscaProdutoAssinatura", {
                                            Produtos: response.Lista
                                        });
                                        $('.jsResultadoProdutoAssinatura').html(Resultado);
    ...
    apagarEnderecoCliente: function(id) {
        $.alertpadrao({
            type: 'html',
            mode: "confirm",
            text: "Confirma a exclusão desse endereço?",
            addClass: "dg-atencao",
            CallBackOK: function() {
                var Itens = id;
                $.ajax({
        ...
        $.ajax({
            ...
            success: function(response) {
                if (parseInt(response.StatusCode) == 0) {
                    ...
                    $.alertpadrao({
                        text: "Consentimentos editados com sucesso!",
                        mode: "alert",
                        addClass: "dg-positivo"
                    });
                } else {
    ...
    BarraMeuPedidoV2: function(lista) {
        if (lista.IsCancelado) {
            var Resultado = TrimPath.processDOMTemplate("#templateBarraMeuPedidoCancelado", {
                Data: ClienteCentral.formatDate(lista.TimeLinePedidoItem[0].Data_HistoricoPedido)
            });
    ...
    OpenModalPix: function(id) {
        $('.dg-modal-pix').css("display", "flex").hide().fadeIn(200, function() {
            setTimeout(function() {
    ...
    GetDadosAssinatura: function(id) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                ClienteCentral.atualizaAssinatura.dados.cartao({
                    bandeira: obj.CartaoAssinatura.Bandeira,
                    final: ultimos9digitos(obj.CartaoAssinatura.Cartao),
                    id: obj.CartaoAssinatura.ID_ClientesCartaoToken
                });
                // ClienteCentral.atualizaAssinatura.dados.frequencia({dias: 0});
                ClienteCentral.atualizaAssinatura.dados.frequencia({
                    dias: obj.Frequencia > 90 ? 30 : obj.Frequencia
                });
...
function imgBandeiraCartaoUsuario(cardN) {
    ...
    if (cardnumber.length < 16 && (cardnumber.length !== 13 && cardnumber.length !== 15 && cardnumber.length !== 14)) {

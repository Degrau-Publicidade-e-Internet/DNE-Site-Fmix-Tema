...
function GetFreteProduto(CEP) {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            var Resultado = TrimPath.processDOMTemplate("#templateFreteProdutoConteudo", {
                RetornoFrete: ResultadoJsonFrete
            });
            $("#templateFreteProduto").html(Resultado);
            abrirModal({
                titulo: 'Cálculo <span>Frete</span> e <span>Prazo</span>',
                id: 'templateFreteProduto'
            });
        ...
        success: function(response) {
            ...
            if (parseInt(response.StatusCode) == 0) {
                if (response.Lista.length > 0) {
                    var _box = TrimPath.processDOMTemplate("#template-genericoSimilar", {
                        SimilaresHtml: response.Msg,
                        SimilaresUrl: response.Lista[0].URLSimilar
                    });
                    abrirModal({

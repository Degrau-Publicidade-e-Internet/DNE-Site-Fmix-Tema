...
var Cliente = {
    efetuarLogin: function (e, obj = null, callback = null) {
        ...
        if (!loginCheck) {
            $.alertpadrao({ type: 'html', text: "Por favor, preencher todos os campos.", addClass: "dg-negativo" });
            return false;
        }
            ...
            success: function (response) {
                ...
                if (response.StatusCode > 0) {
                    ...
                    if ($(".dg-header-minhaconta").length > 0) {
                        ...
                        if (window.location.href.indexOf("/cliente/login/") > -1) {
                            location.href = '/';
                        }
    ...
    logOutCliente: function () {
        ...
        $.alertpadrao({
            type: 'html', mode: "confirm", text: "Você tem certeza que deseja encerrar a sessão?", addClass: "dg-atencao", CallBackOK: function () {
    ...
    esqueciSenha: {
        ...
        ajax: function (el) {
            ...
            var step1 = form.find('[data-form-senha-step="1"]');
            var step2 = form.find('[data-form-senha-step="2"]');
            var step3 = form.find('[data-form-senha-step="3"]');
            
            var ajaxUrl = "/Api/EsqueciSenha/";
            var method = "POST";

            var sendData = null;

            if (step2.hasClass('dg-step-ativo')) {
                item.tipo = form.find('input[name="escolhaAlteracaoSenha"]:checked').val();
                if (item.tipo === "" || typeof item.tipo === "undefined") {
                    $.alertpadrao({
                        type: 'html',
            ...
            } else if (step3.hasClass('dg-step-ativo')) {
                if (self.tipo === "2") {
                    var smsCodeEl = form.find('.jsCodigoSmsInputEsqueciSenha');
                    var smsCode = "";
            ...
            $.ajax({
                ...
                headers: addAntiForgeryToken({}), //<< obrigatorio pessoal
                success: function (response) {
                    $('.dg-header-minhaconta-flex').addClass('dg-no__beforeafter');
                    ...
                    if (response.StatusCode > 0) {
                        ...
                        $.alertpadrao({ type: 'html', text: response.Erro, addClass: "dg-negativo",
                            CallBackOK: function () {
                                if (step3.hasClass('dg-step-ativo')) {
                                    if (self.tipo === "2") {
                                        form.find('.jsCodigoSmsInputEsqueciSenha').first().focus();
                                        $('.jsCodigoSmsInputEsqueciSenha').val('');
                                    }
                                }
                            }
                        });
                        ...
                        if (step1.hasClass('dg-step-ativo')) {
                            step1.hide();
                            step1.removeClass('dg-step-ativo');
                            
                            var Resultado = TrimPath.processDOMTemplate("#templateEsqueciSenha2", {Data: response.Lista});
                            step2.append(Resultado);
                            step2.show();
                            step2.addClass('dg-step-ativo');
        
                            $('.jsHeaderMinhacontaModalCadastro').hide();
                            
                            $('.jsVoltaStep1Senha').on('click', function() {
                                step2.empty();
                                step2.hide();
                                step2.removeClass('dg-step-ativo');
                                
                                step1.show();
                                step1.addClass('dg-step-ativo');
    
                                $('.jsHeaderMinhacontaModalCadastro').show();
                                $('.dg-no__beforeafter').removeClass('dg-no__beforeafter');
                            });
                            
                            form.removeClass('dg-loading');
                            
                        } else if (step2.hasClass('dg-step-ativo')) {
                            step2.hide();
                            step2.removeClass('dg-step-ativo');
    
                            var content = {};
                            content = response.Lista;
                            content.tipo = self.tipo;
    
                            var Resultado = TrimPath.processDOMTemplate("#templateEsqueciSenhaResultado", {Data: content});
                            
                            step3.append(Resultado);
                            step3.show();
                            step3.addClass('dg-step-ativo');
    
                            $('.jsVoltarParaLoginSenha').on('click', function() {
                                step2.empty();
                                step2.removeClass('dg-step-ativo');
                                step3.empty();
                                step3.removeClass('dg-step-ativo');
    
                                step1.show();
                                step1.addClass('dg-step-ativo');
    
                                $('#EsqueciSenhaEmail').removeClass('dg-step-ativo');
    
                                $(".jsHeaderMinhacontaModalEsqueciSenha").hide();
                                $(".jsHeaderMinhacontaModalLogin").css('display', 'flex');
                                $('.jsHeaderMinhacontaModalCadastro').show();
                                $('.dg-no__beforeafter').removeClass('dg-no__beforeafter');
                            });
    
                            if (self.tipo === "1") {
                                $('.jsReenviarEmail').on('click', function() {
                                    if (!self.esperando30s) {
                                        self.esperando30s = true;
                                        form.addClass('dg-loading');

                                        $('.jsReenviarEmail').css({opacity: '0.65'});
                                        $('.jsReenviarEmail').css({cursor: 'not-allowed'});

                                        var counter = 30;
                                        $('.jsReenviarEmailTimer').text("(30s)");
                                        
                                        var intervalo = setInterval(function() {
                                            counter--;
                                            var stringCounter = "(" + (counter < 10 ? "0" + counter : counter) + "s)";
                                            $('.jsReenviarEmailTimer').text(stringCounter);
                                            
                                            if (counter === -1) {
                                                self.esperando30s = false;
                                                $('.jsReenviarEmail').css({opacity: '1'});
                                                $('.jsReenviarEmail').css({cursor: 'pointer'});
                                                $('.jsReenviarEmailTimer').empty();
                                ...
                                $('.jsCodigoSmsInputEsqueciSenha').first().focus();
    
                                $('.jsCodigoSmsInputEsqueciSenha').on('input', function() {
                                ...
                                $('.jsCodigoSmsInputEsqueciSenha').on('keydown', function(e) {
                                ...
                                $('.jsReenviarCodigo').on('click', function() {
                                    if (!self.esperando30s) {
                                        self.esperando30s = true;
                                        form.addClass('dg-loading');

                                        $('.jsReenviarCodigo').css({opacity: '0.65'});
                                        $('.jsReenviarCodigo').css({cursor: 'not-allowed'});

                                        var counter = 30;
                                        $('.jsReenviarCodigoTimer').text("(30s)");
                                        
                                        var intervalo = setInterval(function() {
                                            counter--;
                                            var stringCounter = "(" + (counter < 10 ? "0" + counter : counter) + "s)";
                                            $('.jsReenviarCodigoTimer').text(stringCounter);
                                            
                                            if (counter === -1) {
                                                self.esperando30s = false;
                                                $('.jsReenviarCodigo').css({opacity: '1'});
                                                $('.jsReenviarCodigo').css({cursor: 'pointer'});
                                                $('.jsReenviarCodigoTimer').empty();
                                                clearInterval(intervalo);
                                            }
                                        }, 1000);
                            ...
                            if (self.tipo === "2") {
                                if (typeof response.Lista.url !== "undefined") {
                                    location.href = response.Lista.url;
                                }
                            } else {
                                form.removeClass('dg-loading');
                            }
    ...
    BarraMeuPedidoV2: function (lista) {
        if (lista.IsCancelado) {
            var Resultado = TrimPath.processDOMTemplate("#templateBarraMeuPedidoCancelado", { Data: Cliente.formatDate(lista.TimeLinePedidoItem[0].Data_HistoricoPedido) });
    ...
    CarregaHeader: function() {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                var Resultado = TrimPath.processDOMTemplate("#templateLogadoLogado", { RetornoLogado: response });
    ...
    EfetuarLoginFaceBook: function () {
        ...
        FB.login(function (response) {
            if (response.authResponse) {
                FB.api('/me', { fields: 'name, email, first_name, last_name' },
                ...
                $.alertpadrao({ type: 'html', text: "Usuario não autorizado", addClass: "dg-negativo" });
            }
...
function CadastrarCliente_GetConfirmacao(Origem) {
    ...
    $(".dg-modal-id-templateCadastrarSmsEmail .problemaenvio-email").click(function () {
        ...
        if (!$('.jsIsMobile').is(':visible')) {
            // versão desktop
            Itens = { "Telefone": "", "Email": (Origem === 'cad' ? $("#Email").val() : $('.dg-header [name="LoginUsuario"]').val()) };//$('#Confirma_Email').val(), "ChaveCliente": $("#ChaveCliente").val()
        } else {
            // versão mobile
            Itens = { "Telefone": "", "Email": (Origem === 'cad' ? $("#Email").val() : $('.dg-menuA [name="LoginUsuario"]').val()) };
        }
    ...
    $(".dg-modal-id-templateCadastrarSmsEmail .btConfirma_enviasms").click(function () {
        ...
        if (!$('.jsIsMobile').is(':visible')) {
            // versão desktop
            Itens = { "SMSCode": $('#SMSCode').val(), "Email": (Origem === 'cad' ? $("#Email").val() : $('.dg-header [name="LoginUsuario"]').val()) };
        } else {
            // versão mobile
            Itens = { "SMSCode": $('#SMSCode').val(), "Email": (Origem === 'cad' ? $("#Email").val() : $('.dg-menuA [name="LoginUsuario"]').val()) };
        }
...
function CadastrarCliente_concluido(origem, response) {
    ...
    if (origem === 'cad') {
        ...
        $.alertpadrao({
            text: "Cadastro efetuado com sucesso!", mode: "alert", addClass: "dg-positivo",
            CallBackOK: function () {
...
function CadastrarCliente_GetStatus(Origem) {
    ...
    if (!$('.jsIsMobile').is(':visible')) {
        // versão desktop
        Itens = { "Email": (Origem === 'cad' ? $("#Email").val() : $('.dg-header [name="LoginUsuario"]').val()) };
    } else {
        // versão mobile
        Itens = { "Email": (Origem === 'cad' ? $("#Email").val() : $('.dg-menuA [name="LoginUsuario"]').val()) };
    }
        ...
        success: function (r) {
            ...
            if (resultado != null) {
                if (resultado.IsAtivo === 1 && $(".problemaenvio-email").hasClass("EnvioEmailAguardando")) {
                    ...
                    if (resultado.IsCarrinho === true) { Direciona = DominioCheckOutAjax + "/checkout/"; }
                    ...
                    setTimeout(function () { CadastrarCliente_GetStatus(Origem); }, 2000);
                }
            } else {
...
function CadastrarCliente_SendConfirmacao(Origem, Direcao, oCallBack) {
    ...
    if (!$('.jsIsMobile').is(':visible')) {
        // versão desktop
        Itens = { "Direcao": Direcao, "Email": (Origem === 'cad' ? $("#Email").val() : $('.dg-header [name="LoginUsuario"]').val()) }
    } else {
        // versão mobile
        Itens = { "Direcao": Direcao, "Email": (Origem === 'cad' ? $("#Email").val() : $('.dg-menuA [name="LoginUsuario"]').val()) }
    }
...
var ReqAlertConfirmClientCad = function () {
    $.alertpadrao({
        type: 'html', text: "Cadastro ativado com sucesso.", addClass: "dg-positivo", CallBackOK: function () {

...
function ModalAssinatura(el, id = null) {
    ...
    var Resultado = TrimPath.processDOMTemplate("#template-ModalAssinatura", { Assinatura: obj });      

    abrirModal({titulo:'', classe: 'modalAssinatura', txt: Resultado, startFunction: addEventosModalAssinatura, parameterStartFuncion: [el, id, temAssinatura] });
    ...
    function addEventosModalAssinatura(array) {
        ...
        $('.jsModalAssinaturaEnviaCarrinho').on('click', function (e) {
            ...
            if (dias !== null) {
                ...
                $.alertpadrao({ type: 'html', text: "Por favor, selecione a quantidade de dias.", addClass: "dg-negativo" });
            }
        });
        ...
        $('.jsModalAssinaturaExclui').on('click', function() {
            if (typeof id === 'number' && !Number.isNaN(id)) {
                $.alertpadrao({
                    type: 'html', mode: "confirm", text: "Deseja excluir a assinatura desse produto?", addClass: "dg-atencao", CallBackOK: function () {
        ...
        function enviaAlteracaoAssinatura() {
            setTimeout(function() {
                if (isCheckoutReference) {
                    ...
                    $.alertpadrao({
                        type: 'html', text: modalAtualTemAssinatura ? "Assinatura gravada." : "Assinatura criada.", addClass: "dg-positivo", EventCloseSameOK: true, CallBackOK: function () {
                            fecharModal($('.dg-modal-assinatura')[0]);
                        }
                    });
                } else {

...
$(document).ready(function () {
	...
	if (window.innerWidth > 991) {
		if ($('.dg-produto-img-listagem').children().length > 3) {
	
			var slider2 = tns({
				container: '.dg-produto-img-listagem',
				items: 3,
				axis: "vertical",
				swipeAngle: true,
				speed: 400,
    ...
    if ($(".dg-produto-boxproduto-slide").length > 0) {
		...
		var slider2 = tns({
			container: '.dg-produto-boxproduto-slide',
			slideBy: 'page',
			autoplay: false,
    ...
    $('.jsReceitaFile').on('change', function() {
		var label = $(this).parent('.dg-produto-enviar-receita');
        var btn = $('.jsReceitaBtn');

        btn.text('Enviado Receita...');
        
        if (label.hasClass('dg-ativo')) {
            label.removeClass('dg-ativo');
            label.removeClass('dg-concluido');
            
            setTimeout(function() {
                label.addClass('dg-ativo');
            }, 100)
            setTimeout(function() {
                label.addClass('dg-concluido');
	...
	if ($('.jsProdutoPromocaoTimer').length > 0) {
		var data = $('.jsProdutoPromocaoTimer').attr('data-SaidaProdutoPromocao');
		var dateSpliteFormatado = data.split(' ');
		var dateFinal = dateSpliteFormatado[0] + 'T' + dateSpliteFormatado[1] + '-03:00';

		new countDown({
			el: '.jsProdutoPromocaoTimer', 
			date: dateFinal,

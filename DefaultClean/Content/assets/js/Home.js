...
$(document).ready(function(){
    ...
    $('.dg-home-boxproduto-slide:not(.dg-slide-event)').each(function() {
        var farmaciaOnlineTimer = $('.jsFarmaciaOnlineEventTimer').length > 0 && $('.dg-slide-event').is(':visible');
        ...
        if (farmaciaOnlineTimer && self.hasClass('dg-hide-event')) {
            self.closest('.tns-outer').hide();
        }
    });
...
var sliderBlackFriday;

function GetProdutosOfertaBlackFriday() {
    ...
    if ($('.jsDescontoPromocionaisSlider.dg-slide-event.tns-slider').length > 0) {
        if (sliderBlackFriday !== undefined) {
            sliderBlackFriday.destroy();
        }
    }
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.Lista.Produtos !== '') {
                ...
                sliderBlackFriday = tns({
                    container: sliderEventEl[0],
                    slideBy: 'page',
                    autoplay: false,
                    autoplayButton: false,
                    autoplayButtonOutput: false,
                ...
                $('.jsDescontoPromocionaisSlider.dg-hide-event').closest('.tns-outer').show();

                // $('.jsDescontoPromocionaisSlider.dg-hide-event').each(function(){
                //     this.goTo('first');
                // });
